...
|---------|--------|---------|------|
| 最小列宽 | 8.43 | 5-15 | 字符单位 |
| 最大列宽 | 100 | 30-150 | 字符单位（从50提升到100） |
| 超长文本列宽 | 120 | 80-150 | 超长文本专用宽度 |
| 极长文本列宽 | 120 | 80-200 | 极长文本（>200字符）固定宽度 |
| 文本缓冲 | 3.5 | 1-5 | 字符单位（从2.0提升） |
| 数值缓冲 | 1.6 | 1-3 | 字符单位 |
| 自动换行阈值 | 80 | 30-120 | 触发换行的列宽（从50提升） |
| 智能识别表头 | 是 | 是/否 | 自动判断第一行是否为表头 |
| 智能断行 | 是 | 是/否 | 在标点符号处智能换行 |
| 最大行高 | 200 | 100-400 | 单元格最大行高（像素） |
| 长文本扩展阈值 | 100 | 50-200 | 触发长文本处理的字符数 |
| 多行换行最大行数 | 10 | 3-15 | 多行换行时的最大行数限制 |
...
#### 3.6.0.1 超长文本智能处理策略（新增）
**功能描述**：针对超长文本（如详细说明、备注等），提供智能的显示和布局方案。
...
**处理策略**：
1. **智能列宽扩展**：
   - 普通长文本：最大列宽可扩展至100字符单位
   - 超长文本：最大列宽可扩展至150字符单位
   - 极长文本（>200字符）：固定120字符宽度+强制换行
