...
|---------|--------|---------|------|
| 最小列宽 | 8.43 | 5-15 | 字符单位 |
| 最大列宽 | 100 | 30-150 | 字符单位（提高到100以支持长文本） |
| 超长文本列宽 | 120 | 80-150 | 超长文本专用宽度 |
| 极长文本列宽 | 150 | 100-200 | 极长文本（>150字符）固定宽度 |
| 文本缓冲 | 3.0 | 1-5 | 字符单位（增加缓冲避免截断） |
| 数值缓冲 | 2.0 | 1-3 | 字符单位 |
| 日期缓冲 | 1.0 | 0.5-2 | 字符单位（减少日期缓冲） |
| 自动换行阈值 | 80 | 50-100 | 触发换行的列宽 |
| 智能识别表头 | 是 | 是/否 | 自动判断第一行是否为表头 |
| 智能断行 | 是 | 是/否 | 在标点符号处智能换行 |
| 最大行高 | 100 | 50-150 | 单元格最大行高（像素） |
| 长文本扩展阈值 | 80 | 50-150 | 触发长文本处理的字符数 |
| 多行换行最大行数 | 5 | 3-10 | 多行换行时的最大行数限制 |
