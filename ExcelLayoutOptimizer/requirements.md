...
|---------|--------|---------|------|
| 最小列宽 | 8.43 | 5-15 | 字符单位 |
| 最大列宽 | 50 | 30-100 | 字符单位（适中的默认值） |
| 超长文本列宽 | 80 | 60-120 | 超长文本专用宽度 |
| 极长文本列宽 | 100 | 80-150 | 极长文本（>200字符）固定宽度 |
| 文本缓冲 | 2.0 | 1-3 | 字符单位（适度缓冲） |
| 数值缓冲 | 1.6 | 1-3 | 字符单位 |
| 自动换行阈值 | 50 | 30-80 | 触发换行的列宽 |
| 智能识别表头 | 是 | 是/否 | 自动判断第一行是否为表头 |
| 智能断行 | 是 | 是/否 | 在标点符号处智能换行 |
| 最大行高 | 60 | 30-100 | 单元格最大行高（像素） |
| 长文本扩展阈值 | 100 | 50-200 | 触发长文本处理的字符数 |
| 多行换行最大行数 | 3 | 2-5 | 多行换行时的最大行数限制 |
